...
impl ProviderStore for SqliteProviderStore {
    ...
    async fn get(&self, id: &str) -> anyhow::Result<Option<Provider>> {
        Ok(sqlx::query_as::<_, Provider>(
            "SELECT id, name, vendor, protocol, base_url, COALESCE(default_protocol, protocol) AS default_protocol, COALESCE(protocol_endpoints, '{}') AS protocol_endpoints, preset_key, channel, models_source, capabilities_source, static_models, api_key, COALESCE(auth_mode, 'api_key') AS auth_mode, access_token, refresh_token, expires_at, COALESCE(use_proxy, 0) AS use_proxy, last_test_success, last_test_at, is_active, created_at, updated_at FROM providers WHERE id = ?",
        )
        .bind(id)
    ...
    async fn update(&self, id: &str, input: UpdateProvider) -> anyhow::Result<Provider> {
        ...
        sqlx::query(
            "UPDATE providers SET name=?, vendor=?, protocol=?, base_url=?, default_protocol=?, protocol_endpoints=?, preset_key=?, channel=?, models_source=?, capabilities_source=?, static_models=?, api_key=?, auth_mode=?, access_token=?, refresh_token=?, expires_at=?, use_proxy=?, is_active=?, updated_at=datetime('now') WHERE id=?",
        )
        .bind(name)
        .bind(vendor)
        .bind(&protocol)
